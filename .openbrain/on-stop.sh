#!/usr/bin/env bash
# OpenBrain vault SessionEnd hook.
...
The SessionEnd hook tried to \`git pull --rebase\` and hit conflicts. The rebase was aborted; your local changes are intact but **not committed or pushed**.
...
SUMMARY="$(git diff --cached --name-only | awk -F/ '
  ...
  END {
    ...
    if (gemini)  parts = parts (parts?", ":"") "GEMINI"
    print parts
  }
')"
...
The SessionEnd hook committed locally but \`git push\` failed. Your work is safe in the local repo.
