...
- **Home.md** is the front door; its "Top MOCs" section is auto-regenerated by the SessionEnd hook.
...
- Git is the sole sync mechanism (Obsidian Sync is disabled). All cross-device sync goes through the SessionEnd hook's commit-and-push cycle.
...
- **Never `git commit` or `git push` manually** — the SessionEnd hook handles commits automatically. If you must intervene, ask first.
...
Three hooks handle maintenance automatically (configured in `.gemini/settings.json`):

- **SessionEnd hook** (`.openbrain/on-stop.sh`) — regenerates `Home.md` MOC index, then smart-commits and pushes (skip-if-clean, pull-rebase first, conflict → inbox note).
