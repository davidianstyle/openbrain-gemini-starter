...
description: Scan sent messages across configured Google and Slack accounts to derive a personalized writing-style profile (email vs Slack, by audience size), then update GEMINI.md §6 in place. Run after initial bootstrap or anytime you want to refresh.
---

# /learn-writing-style

Derive your writing-style profile from your real outbound messages and write it into `GEMINI.md §6`. The drafter skills (`/follow-up-draft`, `/daily-brief`, `/process-inbox`) read §6 as the source of truth, so updating §6 propagates automatically.
...
> **Date check first.** Run shell `date "+%Y-%m-%d"` to anchor "recent" before any other work.

### 1. Inventory channels

From GEMINI.md §11, list configured `google_*` accounts and `slack_*` workspaces. Skip any whose MCP server is unreachable; note skipped channels in the final report.
...
Render results in `GEMINI.md §6` shape. The exact bullet structure must match what's already there so re-runs are clean overwrites:
...
Print the proposed §6 body in chat, plus per-channel sample counts. Then ask the user to confirm:
...
Replace the body of `## 6. Writing assistant guidance` in `GEMINI.md` (overwrite from the heading to the next H2 or EOF). Preserve the section header and everything else in the file. Do **not** append.

If `$2` is `--dry-run`, skip step 8 entirely.

### 9. Update memory (if applicable)

If your environment maintains a writing-style memory entry, refresh it to mirror the new §6 profile, citing GEMINI.md §6 as the source of truth.
