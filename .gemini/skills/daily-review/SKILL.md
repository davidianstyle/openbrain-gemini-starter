...
- **Asana — create new.** `asana_create_task` on the correct workspace with title, due date, and a `notes` field linking back to the source (interaction note path or person name). Write the returned `gid` + `workspace` back into frontmatter of any corresponding vault note per GEMINI.md §5.
...
- Follow GEMINI.md §5 Asana routing rules strictly: `#asana/work` → `asana_work`, `#asana/personal` → `asana_personal`, never deprecated connectors.
