...
2. **Collect touchpoints across all accounts.** Fan out all four sources in a single tool-use block — every `google_*`, every `slack_*`, and Fathom. All calls must run concurrently.
   ...
   - **Fathom** — `mcp_fathom_list_meetings` over the window. Extract `calendar_invitees` and their email domains. Fathom's `is_external: true` flag is high-signal for outside organizations.
