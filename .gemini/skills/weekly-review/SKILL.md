...
6. **Refresh `Dashboard.md` week-ahead view.** This skill owns the dashboard's forward-looking sections — rebuild them from fresh Asana + calendar data for the **upcoming** week (Mon → Sun starting tomorrow if running on a Sunday/Monday morning, otherwise the next 7 days from today). If `Dashboard.md` is missing, log a warning and skip step 6. Procedure:
