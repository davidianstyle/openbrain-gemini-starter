---
name: process-inbox
description: Triage + Inbox/ together with fresh items in Gmail and Slack across all accounts. Proposes classification per GEMINI.md §4 and surfaces anything needing a reply.
---

# /process-inbox

Run the GEMINI.md §4 inbox triage workflow across all sources.
...
1. **Vault inbox.** Read every `.md` file in `+ Inbox/`. For each, propose classification (atomic / literature / task / project kickoff / ephemeral) and destination, per GEMINI.md §4.
...
7. When moving a note, update backlinks as GEMINI.md §4 requires.
